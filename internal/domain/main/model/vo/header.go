...
func (h Header) Projection(projection *configVO.Projection) Header {
...
func (h Header) Map(mapper *configVO.Mapper) Header {
...
func (h Header) Modify(modify *Modify) Header {
	...
	var newValue []string
	if helper.IsNotEqualTo(modify.Action(), configEnum.ModifierActionDel) {
		newValue = modify.ValueAsSliceOfString()
	}

	// modificamos a partir de uma ação
	switch modify.Action() {
	case configEnum.ModifierActionAdd:
		return h.AddAll(modify.Key(), newValue)
	case configEnum.ModifierActionApd:
		return h.Append(modify.Key(), newValue)
	case configEnum.ModifierActionSet:
		return h.SetAll(modify.Key(), newValue)
	case configEnum.ModifierActionRpl:
		return h.Replace(modify.Key(), newValue)
	case configEnum.ModifierActionDel:
		return h.Delete(modify.Key())
	default:
		return h
	}
}
...
func (h Header) projectionAddition(projection *configVO.Projection) Header {
...
func (h Header) projectionRejection(projection *configVO.Projection) Header {
