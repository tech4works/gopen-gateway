...
import (
	...
	"github.com/GabrielHCataldo/gopen-gateway/internal/domain/config/model/enum"
	"strings"
)
...
func (u UrlPath) Modify(modify *Modify) UrlPath {
	...
	case enum.ModifierActionSet:
		return u.SetParam(modify.Key(), newValue)
	case enum.ModifierActionRpl:
		return u.ReplaceParam(modify.Key(), newValue)
	case enum.ModifierActionDel:
		return u.DeleteParam(modify.Key())
	default:
		return u
	}
}
