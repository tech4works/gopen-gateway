...
func NewHttpBackendRequest(backend *configVO.Backend, httpRequest *HttpRequest, httpResponse *HttpResponse,
) *HttpBackendRequest {
...
func newBackendRequestPath(backend *configVO.Backend, httpRequest *HttpRequest, httpResponse *HttpResponse) UrlPath {
	...
	if helper.IsNotNil(backendRequest) {
		...
		for _, modifier := range backendRequest.ParamModifiers() {
			path = path.Modify(NewModify(&modifier, httpRequest, httpResponse))
		}
	}
...
func newBackendRequestHeader(backend *configVO.Backend, httpRequest *HttpRequest, httpResponse *HttpResponse) Header {
...
func newBackendRequestQuery(backend *configVO.Backend, httpRequest *HttpRequest, httpResponse *HttpResponse) Query {
	...
	for _, modifier := range backendRequest.QueryModifiers() {
		query = query.Modify(NewModify(&modifier, httpRequest, httpResponse))
	}
...
func newBackendRequestBody(backend *configVO.Backend, httpRequest *HttpRequest, httpResponse *HttpResponse) *Body {
	// inicializamos o backendRequest para ser utilizado para configurar o query
	backendRequest := backend.Request()
