...
type EndpointResponseJson struct {
	...
	Encode configEnum.Encode `json:"encode,omitempty" validate:"omitempty,enum"`
	// todo:
	Nomenclature configEnum.Nomenclature `json:"nomenclature,omitempty" validate:"omitempty,enum"`
	// todo:
...
type BackendResponseJson struct {
	Apply              configEnum.BackendResponseApply `json:"apply,omitempty" validate:"omitempty,enum"`
...
type ModifierJson struct {
	...
	Action configEnum.ModifierAction `json:"action,omitempty" validate:"required,enum"`
...
func (j EndpointResponseJson) CountAllDataTransforms() (count int) {
	if j.Aggregate {
		count++
	}
	if j.OmitEmpty {
	...
	if j.Encode.IsEnumValid() {
		count++
	}
	if j.Nomenclature.IsEnumValid() {
		count++
	}
	return count
}
