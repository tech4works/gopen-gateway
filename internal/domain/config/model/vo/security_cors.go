...
func (s SecurityCors) AllowHeaders(httpHeader http.Header) (err error) {
	...
	for key := range httpHeader {
		// caso seja X-Forwarded-For ou X-Trace-Id, ignoramos
		if helper.Equals(key, consts.XForwardedFor) && helper.IsNotEqualToIgnoreCase(key, consts.XTraceId) {
			continue
		} else if helper.NotContains(s.allowHeaders, key) {
