...
func (e EndpointCache) CanRead(httpMethod string, control enum.CacheControl) bool {
	...
	return helper.IsNotEqualTo(enum.CacheControlNoCache, control) && e.AllowMethod(httpMethod)
}

func (e EndpointCache) CanWrite(httpMethod string, httpStatusCode int, control enum.CacheControl) bool {
	// verificamos se ta ativo
	if e.Disabled() {
		return false
	}
	...
	return helper.IsNotEqualTo(enum.CacheControlNoStore, control) && e.AllowMethod(httpMethod) &&
		e.AllowStatusCode(httpStatusCode)
}
